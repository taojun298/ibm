...
<body onLoad="clockon(bgclock)">&quot;&quot;=im&nbsp; 
@i<%@include file="banner.jsp"%>
          ...
          <tr>
            <td valign="top"><table width="100%" height="35" border="1" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF" bordercolorlight="#FFFFFF" bordercolordark="#F6B83B" bgcolor="#FFFFFF">
                <tr align="center" bgcolor="#e3F4F7">
                  <td width="20%" height="25" bgcolor="#FFF9D9">图书名称</td>
                  <td width="11%" bgcolor="#FFF9D9">借阅时间</td>
                  <td width="11%" bgcolor="#FFF9D9">应还时间</td>
                  <td width="14%" bgcolor="#FFF9D9">出版社</td>
                  <td width="11%" bgcolor="#FFF9D9">书架</td>
                  <td bgcolor="#FFF9D9">定价(元)</td>
                  <td width="12%" bgcolor="#FFF9D9">过期罚款(元)</td>
                  
                   
                  <td width="11%" bgcolor="#FFF9D9"><input name="Button22" type="button" class="btn_grey" value="完成归还" onClick="window.location.href='bookBack.jsp'"></td>
                </tr>
                <%
int id=0;
String bookname="";
...
if(coll!=null && !coll.isEmpty()){
	...
	Date dateU=new Date();
	//字符串切割成数组
	String[] array = backTime.split("-");
	backU = new Date(Integer.parseInt(array[0]) - 1900,Integer.parseInt(array[1])-1,Integer.parseInt(array[2]));
	
	if(dateU.getTime()-backU.getTime()  > 0)
	{
		penalty=Float.parseFloat(df.format(((dateU.getTime()-backU.getTime())/1000/3600/24 ) * a).toString());
	}
...
%>
                <tr>
                  ...
                  <td width="12%" align="center">&nbsp;<%=penalty %></td>
                  <td width="11%" align="center"><a href="borrow.do?action=bookback&barcode=<%=barcode%>&id=<%=id%>&operator=<%=chStr.toChinese(manager)%>">归还</a>&nbsp;</td>
                </tr>
                <% }
}%>
            </table>
