...
function check(form){
	if(form.name.value==""){
		alert("请输入用户名称!");form.name.focus();return;
	}
	if(form.pwd.value==""){
		alert("请输入用户密码!");form.pwd.focus();return;
	}
	if(form.pwd1.value==""){
		alert("请确认用户密码!");form.pwd1.focus();return;
	}		
	if(form.pwd1.value!=form.pwd.value){
		alert("您两次输入的用户密码不一致，请重新输入!");form.pwd.focus();return;
	}
	form.submit();
}
</script>
...
<table width="292" height="175" border="0" cellpadding="0" cellspacing="0" background="Images/subBG.jpg">
  <tr>
	...
	<table height="123"  border="0" cellpadding="0" cellspacing="0">
      <tr>
        <td width="83" height="30" align="center">用户名称：</td>
        <td width="192">
